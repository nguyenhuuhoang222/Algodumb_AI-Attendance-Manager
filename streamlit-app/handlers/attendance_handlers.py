...
def handle_successful_attendance(response, attendance_type):
    ...
    <div class="success-card">
        <h4 style="margin: 0 0 10px 0;">Attendance Recorded</h4>
        <p><strong>Student ID:</strong> {student_id}</p>
        <p><strong>Name:</strong> {student_info.get('full_name', 'N/A')}</p>
        <p><strong>Class:</strong> {student_info.get('class_name', 'N/A')}</p>
...
def handle_failed_attendance(response):
    ...
    st.warning("Attendance submission failed!")
...
def auto_continue_workflow():
    ...
    st.info("System is ready for the next person...")
